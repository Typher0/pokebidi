    ...
    for (i = 0; i < gSpecialVar_0x8004; i++)
    {
        if (myLinkPlayerNumber != i)
        {
            ...
            case VERSION_EMERALD:
                if (gLinkPlayers[i].gender == 0)
                    linkSpriteId = OBJ_EVENT_GFX_RIVAL_LUCAS_NORMAL;
                else
        ...
        graphicsId == OBJ_EVENT_GFX_RIVAL_DAWN_NORMAL)
    {
        ...
        if (obj != OBJECT_EVENTS_COUNT)
        {
            ...
            case OBJ_EVENT_GFX_LINK_RS_MAY:
                ...
                break;
            case OBJ_EVENT_GFX_RIVAL_LUCAS_NORMAL:
                LoadPalette(gObjectEventPal_Lucas, OBJ_PLTT_ID(adjustedPaletteNum), PLTT_SIZE_4BPP);
                break;
...
void ForcePlayerOntoBike(void)
{
    ...
    Overworld_SetSavedMusic(IS_FRLG ? MUS_HG_CYCLING : MUS_DP_CYCLING);
    Overworld_ChangeMusicTo(IS_FRLG ? MUS_HG_CYCLING : MUS_DP_CYCLING);
}
