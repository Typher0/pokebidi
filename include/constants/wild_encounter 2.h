#ifndef GUARD_CONSTANTS_WILD_ENCOUNTER_H
#define GUARD_CONSTANTS_WILD_ENCOUNTER_H

#define LAND_WILD_COUNT     12
#define WATER_WILD_COUNT    5
#define ROCK_WILD_COUNT     5
#define FISH_WILD_COUNT     10
#define HIDDEN_WILD_COUNT   3
#define HONEY_WILD_COUNT    5
#define HEADBUTT_WILD_COUNT 5
#define LAND_OWE_WILD_COUNT 12
#define WATER_OWE_WILD_COUNT 5
#define LAND_DOUBLE_WILD_COUNT 4
#define WATER_DOUBLE_WILD_COUNT 4

#define NUM_ALTERING_CAVE_TABLES 11

#define WILD_CHECK_REPEL    (1 << 0)
#define WILD_CHECK_KEEN_EYE (1 << 1)

#endif // GUARD_CONSTANTS_WILD_ENCOUNTER_H
